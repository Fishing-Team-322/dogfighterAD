...
GPO metadata/links, supported SYSVOL settings and deterministic `.dogad` serialization are also implemented. SYSVOL path scope and streaming byte limits are enforced before/while I/O. Blocking SYSVOL filesystem calls are isolated in a disposable helper process with per-operation deadlines so a stalled SMB/filesystem call cannot indefinitely hold the collection stage. A runnable scan CLI, live AD integration, analysis rules and reports are **not implemented yet**. See the [foundation review](docs/reviews/2026-09-06-foundation-review.md) for verified results and remaining live-integration gaps.
