...
- This comparison confirms that the DC-side SYSVOL share, policy directories and domain-style SYSVOL authority are functional from the comparison VM. It does not by itself prove that every remaining failure on MAIN is caused by one specific Windows authentication mechanism.
...
- Absence of that registry path does **not** mean SYSVOL/NETLOGON UNC hardening is disabled. Default Windows protections can still apply. Any future product-owned SMB transport must preserve the relevant server-authentication, signing and scope guarantees rather than bypass them.
- `Get-SmbConnection` on MAIN showed IPC connections to both `dc.mini.lab` and `mini.lab` as `UserName=MAIN\del1m`, SMB dialect 3.1.1, signed and not encrypted, even after an explicit `net use ... /user:MINILAB\alice *` command reported success. This is evidence that the local SMB session/authentication context differs from the successful VM path; it is not by itself proof of which authentication mechanism is selected for every tree connect.
...
- A fresh `runas /netonly /user:MINILAB\alice powershell.exe` session on MAIN still could not read either `\\dc.mini.lab\SYSVOL\mini.lab\Policies` or `\\mini.lab\SYSVOL\mini.lab\Policies`; both returned `PathNotFound`. Therefore merely moving the explicit domain identity into a network-only logon session did not make the MAIN path behave like the comparison VM.
- In that network-only session, TCP/88 to `dc.mini.lab` succeeded from `192.168.57.1`, so the Kerberos service port itself was reachable. However, `klist` showed zero cached tickets and `klist get cifs/dc.mini.lab` failed (`LsaCallAuthenticationPackage` 0x51f; `klist` 0xc000005e). The exact cause of that ticket acquisition failure remains to be localized; workgroup membership alone is not sufficient to explain or prove the failure.
...
- The live evidence therefore establishes a client/environment difference between MAIN and WS, but does not justify reducing it to `WORKGROUP => no Kerberos`. DNS, realm/KDC discovery or configuration, account context, Windows logon semantics, server identity and policy can still affect the result.
- Do not disable UNC hardening, weaken SMB signing, or relax scanner SYSVOL scope to make MAIN pass.

## Portability follow-up

The production target should not require a domain-joined Windows scanner. The next bounded investigation is a proof-of-concept that acquires Kerberos credentials explicitly and reads one known `GPT.INI` over an application-owned SMB client from both (1) the non-domain MAIN workstation and (2) Linux. Candidate libraries such as SMBLibrary and Kerberos.NET are evaluation candidates only; their interoperability, security properties, licensing/packaging and exact AD behavior must be proven by the prototype before they are integrated into DogfighterAD.

The proof-of-concept must preserve server identity validation, SMB signing requirements, the existing approved SYSVOL/GPO path scope, byte limits and hard operation deadlines. A successful prototype is evidence only for the tested path; it does not make later `audit-full` failures automatically product defects because permissions, DNS, directory state and lab-specific behavior can still contribute.
...
The exact named-server flag alone does not configure referral chasing. Microsoft's LDAP hostname guidance covers avoiding initial locator lookups; this change separately controls follow-on referral behavior.
...
Only minimal was tested in this workstation run. Explicit `-u` credentials still apply to LDAP only; the current filesystem-backed `audit-full` SYSVOL path requires a suitable host SMB security context. The ordinary VM has passed the manual SYSVOL prerequisite for both the DC FQDN and domain-style UNC authorities; MAIN has not. The portability prototype is intended to determine whether DogfighterAD can own this authentication/SMB boundary cross-platform instead of inheriting host SMB session state.
