...
- The live evidence is consistent with a client-side SYSVOL mutual-authentication / UNC-hardening prerequisite being unsatisfied on the workgroup workstation, but the comparison is not yet complete until the VM's domain-membership state and the MAIN Kerberos ticket/KDC path are checked. Do not disable UNC hardening or weaken SMB signing to make the lab pass.
