...
- The primary workstation is a workgroup machine (`Name=MAIN`, `PartOfDomain=False`, `Domain=WORKGROUP`) and has no configured `HKLM\SOFTWARE\Policies\Microsoft\Windows\NetworkProvider\HardenedPaths` values. This makes an explicitly configured Hardened UNC Paths policy an unlikely explanation for the local failure.
- `Get-SmbConnection` on the primary workstation showed the existing `IPC$` connections to both `dc.mini.lab` and `mini.lab` as `UserName=MAIN\del1m`, SMB dialect 3.1.1, signed and not encrypted. This does not match the intended MINILAB\alice network identity and indicates that the live SMB connection state is being reused under the local account context.
- An explicit tree connect `net use \\dc.mini.lab\SYSVOL /user:MINILAB\alice *` returned system error 5 (`Access is denied`). The equivalent `net use \\mini.lab\SYSVOL /user:MINILAB\alice *` returned system error 67 (`The network name cannot be found`). These errors are now the narrowest live evidence for the primary-workstation SYSVOL problem.
- The next validation step should isolate the lab SMB connection state or use a fresh network-only logon session rather than changing DC configuration, DNS, or scanner SYSVOL scope.
