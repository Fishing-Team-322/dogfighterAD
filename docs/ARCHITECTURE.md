...
The foundation is not yet a complete AD scanner. Current network collection covers RootDSE discovery, default-domain metadata, users/groups/computers/OUs, group memberships including primary groups and foreign security principals, and configured local trust relationships. ACLs/security descriptors, GPO data, SYSVOL settings and ADCS collection are planned next.
