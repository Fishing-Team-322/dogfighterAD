...
public sealed class RuleEngineContractTests
{
    ...
    [Fact]
    public void CatalogContainsEightyUniqueDocumentedExecutableRules()
    {
        var pack = BuiltInRulePack.Create(); Assert.Equal(80, pack.Count);
        Assert.Equal(80, pack.Select(r => r.Metadata.Id).Distinct(StringComparer.Ordinal).Count());
        ...
        Assert.Equal(80, report.Rules.Count); Assert.DoesNotContain(report.Evaluations, e => e.Outcome == RuleOutcome.Error);
